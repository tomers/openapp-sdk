/*
OpenApp API

IoT Device Management (Gates/Doors/Provisioning) API

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the InviteScheduleEntrySnapshot type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InviteScheduleEntrySnapshot{}

// InviteScheduleEntrySnapshot struct for InviteScheduleEntrySnapshot
type InviteScheduleEntrySnapshot struct {
	Id               string                 `json:"id"`
	InviteRecurrence interface{}            `json:"invite_recurrence,omitempty"`
	IsEnabled        bool                   `json:"is_enabled"`
	Name             NullableString         `json:"name,omitempty"`
	Schedule         InviteScheduleSnapshot `json:"schedule"`
	ValidFrom        string                 `json:"valid_from"`
	ValidTo          string                 `json:"valid_to"`
}

type _InviteScheduleEntrySnapshot InviteScheduleEntrySnapshot

// NewInviteScheduleEntrySnapshot instantiates a new InviteScheduleEntrySnapshot object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInviteScheduleEntrySnapshot(id string, isEnabled bool, schedule InviteScheduleSnapshot, validFrom string, validTo string) *InviteScheduleEntrySnapshot {
	this := InviteScheduleEntrySnapshot{}
	this.Id = id
	this.IsEnabled = isEnabled
	this.Schedule = schedule
	this.ValidFrom = validFrom
	this.ValidTo = validTo
	return &this
}

// NewInviteScheduleEntrySnapshotWithDefaults instantiates a new InviteScheduleEntrySnapshot object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInviteScheduleEntrySnapshotWithDefaults() *InviteScheduleEntrySnapshot {
	this := InviteScheduleEntrySnapshot{}
	return &this
}

// GetId returns the Id field value
func (o *InviteScheduleEntrySnapshot) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *InviteScheduleEntrySnapshot) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *InviteScheduleEntrySnapshot) SetId(v string) {
	o.Id = v
}

// GetInviteRecurrence returns the InviteRecurrence field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InviteScheduleEntrySnapshot) GetInviteRecurrence() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.InviteRecurrence
}

// GetInviteRecurrenceOk returns a tuple with the InviteRecurrence field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InviteScheduleEntrySnapshot) GetInviteRecurrenceOk() (*interface{}, bool) {
	if o == nil || IsNil(o.InviteRecurrence) {
		return nil, false
	}
	return &o.InviteRecurrence, true
}

// HasInviteRecurrence returns a boolean if a field has been set.
func (o *InviteScheduleEntrySnapshot) HasInviteRecurrence() bool {
	if o != nil && !IsNil(o.InviteRecurrence) {
		return true
	}

	return false
}

// SetInviteRecurrence gets a reference to the given interface{} and assigns it to the InviteRecurrence field.
func (o *InviteScheduleEntrySnapshot) SetInviteRecurrence(v interface{}) {
	o.InviteRecurrence = v
}

// GetIsEnabled returns the IsEnabled field value
func (o *InviteScheduleEntrySnapshot) GetIsEnabled() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsEnabled
}

// GetIsEnabledOk returns a tuple with the IsEnabled field value
// and a boolean to check if the value has been set.
func (o *InviteScheduleEntrySnapshot) GetIsEnabledOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsEnabled, true
}

// SetIsEnabled sets field value
func (o *InviteScheduleEntrySnapshot) SetIsEnabled(v bool) {
	o.IsEnabled = v
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *InviteScheduleEntrySnapshot) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *InviteScheduleEntrySnapshot) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *InviteScheduleEntrySnapshot) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *InviteScheduleEntrySnapshot) SetName(v string) {
	o.Name.Set(&v)
}

// SetNameNil sets the value for Name to be an explicit nil
func (o *InviteScheduleEntrySnapshot) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *InviteScheduleEntrySnapshot) UnsetName() {
	o.Name.Unset()
}

// GetSchedule returns the Schedule field value
func (o *InviteScheduleEntrySnapshot) GetSchedule() InviteScheduleSnapshot {
	if o == nil {
		var ret InviteScheduleSnapshot
		return ret
	}

	return o.Schedule
}

// GetScheduleOk returns a tuple with the Schedule field value
// and a boolean to check if the value has been set.
func (o *InviteScheduleEntrySnapshot) GetScheduleOk() (*InviteScheduleSnapshot, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Schedule, true
}

// SetSchedule sets field value
func (o *InviteScheduleEntrySnapshot) SetSchedule(v InviteScheduleSnapshot) {
	o.Schedule = v
}

// GetValidFrom returns the ValidFrom field value
func (o *InviteScheduleEntrySnapshot) GetValidFrom() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ValidFrom
}

// GetValidFromOk returns a tuple with the ValidFrom field value
// and a boolean to check if the value has been set.
func (o *InviteScheduleEntrySnapshot) GetValidFromOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ValidFrom, true
}

// SetValidFrom sets field value
func (o *InviteScheduleEntrySnapshot) SetValidFrom(v string) {
	o.ValidFrom = v
}

// GetValidTo returns the ValidTo field value
func (o *InviteScheduleEntrySnapshot) GetValidTo() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ValidTo
}

// GetValidToOk returns a tuple with the ValidTo field value
// and a boolean to check if the value has been set.
func (o *InviteScheduleEntrySnapshot) GetValidToOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ValidTo, true
}

// SetValidTo sets field value
func (o *InviteScheduleEntrySnapshot) SetValidTo(v string) {
	o.ValidTo = v
}

func (o InviteScheduleEntrySnapshot) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	if o.InviteRecurrence != nil {
		toSerialize["invite_recurrence"] = o.InviteRecurrence
	}
	toSerialize["is_enabled"] = o.IsEnabled
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	toSerialize["schedule"] = o.Schedule
	toSerialize["valid_from"] = o.ValidFrom
	toSerialize["valid_to"] = o.ValidTo
	return toSerialize, nil
}

func (o *InviteScheduleEntrySnapshot) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"is_enabled",
		"schedule",
		"valid_from",
		"valid_to",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varInviteScheduleEntrySnapshot := _InviteScheduleEntrySnapshot{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varInviteScheduleEntrySnapshot)

	if err != nil {
		return err
	}

	*o = InviteScheduleEntrySnapshot(varInviteScheduleEntrySnapshot)

	return err
}

type NullableInviteScheduleEntrySnapshot struct {
	value *InviteScheduleEntrySnapshot
	isSet bool
}

func (v NullableInviteScheduleEntrySnapshot) Get() *InviteScheduleEntrySnapshot {
	return v.value
}

func (v *NullableInviteScheduleEntrySnapshot) Set(val *InviteScheduleEntrySnapshot) {
	v.value = val
	v.isSet = true
}

func (v NullableInviteScheduleEntrySnapshot) IsSet() bool {
	return v.isSet
}

func (v *NullableInviteScheduleEntrySnapshot) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInviteScheduleEntrySnapshot(val *InviteScheduleEntrySnapshot) *NullableInviteScheduleEntrySnapshot {
	return &NullableInviteScheduleEntrySnapshot{value: val, isSet: true}
}

func (v NullableInviteScheduleEntrySnapshot) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInviteScheduleEntrySnapshot) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
