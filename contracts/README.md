...
- **OpenAPI** remains the canonical schema (`packages/api-spec/openapi.json`), enforced by `just sdk openapi-check`.
...
- **Broker publish (optional)** — after contract tests, CI can run `packages/sdk/python/scripts/publish_pacts_to_broker.sh` when `PACT_BROKER_BASE_URL` is set (see `packages/sdk/docs/CI_FANOUT.md`).
